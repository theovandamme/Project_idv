...
ColumnOrientedAccumulator.prototype['@@transducer/result'] = ramda.identity;
...
GeoJSONAccumulator.prototype['@@transducer/result'] = ramda.identity;
...
const _isArrayLike = ramda.curryN(1, function isArrayLike (x) {
...
const count = _dispatchableSummarizer(_xcount, function count (list) {
  return _isArrayLike(list)
    ? list.length
    : result(ramda.reduce(step, init(), list))
});

const init$1 = () => -Infinity;
const result$1 = ramda.identity;
...
const max = _dispatchableSummarizer(_xmax, function (list) {
  return result$1(ramda.reduce(step$1, init$1(), list))
});
...
const mean = _dispatchableSummarizer(_xmean, function (list) {
  return result$2(ramda.reduce(step$2, init$2(), list))
});
...
const min = _dispatchableSummarizer(_xmin, function (list) {
  return result$3(ramda.reduce(step$3, init$3(), list))
});
...
const mode = _dispatchableSummarizer(_xmode, function (list) {
  return result$4(ramda.reduce(step$4, init$4(), list))
});

const init$5 = () => 0;
const result$5 = ramda.identity;
...
const sum = _dispatchableSummarizer(_xsum, function (list) {
  return result$5(ramda.reduce(step$5, init$5(), list))
});
...
const _xarrange = ramda.curryN(2, function _xarrange (arrangeInstructions, xf) {
  return new XArrange(arrangeInstructions, xf)
});

const arrange = ramda.curryN(2, _dispatchable([], _xarrange,
...
XArrange.prototype['@@transducer/result'] = function () {
  ...
  return this.xf['@@transducer/result'](ramda.reduce(
...
const _stepCatArray = {
  ...
  '@@transducer/result': ramda.identity
};
...
const _stepCatObject = {
  ...
  '@@transducer/step': function (result, input) {
    return Object.assign(
      result,
      _isArrayLike(input) ? ramda.objOf(input[0], input[1]) : input
    )
  },
  '@@transducer/result': ramda.identity
};
...
const _xnestBy = ramda.curryN(3, function _xnestBy (nestInstructions, by, xf) {
  return new XNestBy(nestInstructions, by, xf)
});

const nestBy = ramda.curryN(3, _dispatchable([], _xnestBy,
...
const _xfilterBy = ramda.curryN(4, function _xfilterBy (summariseFn, predicate, by, xf) {
  return _isReducable(summariseFn)
    ? _xfilterByReducable(summariseFn, predicate, by, xf)
    : _xfilterByIrreducable(summariseFn, predicate, by, xf)
});

const filterBy = ramda.curryN(4, _dispatchable([], _xfilterBy,
...
const _xpivotLonger = ramda.curryN(2, function _xpivotLonger (pivotInstructions, xf) {
  return new XPivotLonger(pivotInstructions, xf)
});

const pivotLonger = ramda.curryN(2, _dispatchable([], _xpivotLonger,
...
const _xpivotWider = ramda.curryN(2, function _xpivotWider (pivotInstructions, xf) {
  return new XPivotWider(pivotInstructions, xf)
});

const pivotWider = ramda.curryN(2, _dispatchable([], _xpivotWider,
  function (pivotInstructions, df) {
    return ramda.into(
      [],
...
const _xslice = ramda.curryN(2, function _xslice (indices, xf) {
  return new XSlice(indices, xf)
});

const slice = ramda.curryN(2, _dispatchable([], _xslice,
  function (indices, df) {
    return ramda.into(
      [],
      slice(indices),
...
const _xunnest = ramda.curryN(3, function _xunnest (nestColName, nestWrapper, xf) {
  return new XUnnest(nestColName, nestWrapper, xf)
});

const unnest = ramda.curryN(3, _dispatchable([], _xunnest,
