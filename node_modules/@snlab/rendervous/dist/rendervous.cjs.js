...
var colortranslator = require('colortranslator');
...
function fitScales (bbox) {
  ...
  const scaleX = d3Scale.scaleLinear().domain(domainX);
  const scaleY = d3Scale.scaleLinear().domain(domainY);
...
class SpatialIndex {
  constructor (interactionHandler, getMark, getLayer) {
    this._rbush = new RBush__default['default']();
...
function parseAesthetics$2 (props, parsedProps) {
  ...
  parsedProps.curve = fallback(props.curve, d3Shape.curveLinear);
  return parseAesthetics$1(props, parsedProps)
}
...
function toRGBA (_color, opacity) {
  ...
  const colorTranslator = new colortranslator.ColorTranslator(color);
...
function mergeProps (lineCoordinates, props) {
  ...
  mergedProps.curve = d3Shape.curveLinear;
...
function createScale (scale, range, addInvert) {
  if (scale.constructor === Array) {
    return d3Scale.scaleLinear().domain(scale).range(range)
  }
...
function createScale$1 (c, r) {
  return d3Scale.scaleLinear().domain([0, 1]).range([c, r + c])
}
